...
void init_velocidad() {
  ...
  /*for (int i = 0; i < n_cuerpos; i++) {
    ...
    cuerpos[i].vX = -vel_compartida * (cuerpos[i].pY * r_inverso +
  random_menos1_a_1(generator)); cuerpos[i].vY = vel_compartida * (cuerpos[i].pX
  * r_inverso + random_menos1_a_1(generator));
...
void Energia_Pmomento_Lmomemnto() {
  ...
  long double V = 0.0L;
  long double V_cruzado = 0.0L;
  for (int j = 0; j < n_cuerpos; j++) {

    long double sqrt_x2_mas_y2 =
        sqrt(pow(cuerpos[j].pX, 2) + pow(cuerpos[j].pY, 2));
    ...
    V += cuerpos[j].masa * G / sqrt_x2_mas_y2;
    // potencial cruzado
    for (int i = 0; i < j; i++) {
      long double x2_menos_y2 = pow((cuerpos[i].pX - cuerpos[j].pX), 2) +
                                pow((cuerpos[i].pY - cuerpos[j].pY), 2);
      long double sqrt_x2_menos_y2 = sqrt(x2_menos_y2);
      V_cruzado += G * cuerpos[i].masa * cuerpos[j].masa / sqrt_x2_menos_y2;
    }
  }
